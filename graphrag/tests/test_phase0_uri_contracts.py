...
class TestUriContractEquivalencePhase0(unittest.TestCase):
    ...
    def test_overloaded_function_uri_equivalence(self) -> None:
        ...
        self.assertEqual(len({e.global_uri for e in overloads}), 1)
        self.assertEqual(len({e.function_sig_hash for e in overloads}), 2)
        scip_uri = scip_symbol_to_global_uri(
            scip_symbol="cxx . . $ add(2222).",
            file_path="overloaded_functions.cpp",
            repo_name=self.repo_name,
            kind=0,
        )
        self.assertTrue(all(entity.global_uri == scip_uri for entity in overloads))
